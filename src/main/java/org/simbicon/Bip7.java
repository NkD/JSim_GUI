...
public class Bip7 {
    ...
    public void resetBiped() {
        ...
        System.arraycopy(copyState, 0, state, 0, nrStates);
        lostControl = false;
    }
     ...
     */
    public void bip7_PointForce(int i1, int i2, float gndHeight, Point2D f) {
        float KpGnd = 100000;
        float KdGnd = 6000;
        float x     = monState[i1];
    ...
    public void drawBiped(Graphics g, Matrix3x3 transform) {
        ...
        pp = transform.multiplyBy(new Point2D(-0.04, 0.02));
        p.addPoint((int) pp.x, -(int) pp.y);
        pp = transform.multiplyBy(new Point2D(0.04, 0.02));
        p.addPoint((int) pp.x, -(int) pp.y);
        pp = transform.multiplyBy(new Point2D(0.04, -0.45));
        p.addPoint((int) pp.x, -(int) pp.y);
        pp = transform.multiplyBy(new Point2D(-0.04, -0.45));
        p.addPoint((int) pp.x, -(int) pp.y);
        ...
        pp = transform.multiplyBy(new Point2D(-0.025, 0.02));
        p.addPoint((int) pp.x, -(int) pp.y);
        pp = transform.multiplyBy(new Point2D(0.025, 0.02));
        p.addPoint((int) pp.x, -(int) pp.y);
        pp = transform.multiplyBy(new Point2D(0.025, -0.45));
        p.addPoint((int) pp.x, -(int) pp.y);
        pp = transform.multiplyBy(new Point2D(-0.025, -0.45));
        p.addPoint((int) pp.x, -(int) pp.y);
        g.setColor(new Color(100, 100, 100));
        ...
        pp        = transform.multiplyBy(new Point2D(-0.04, 0.0));
        p.addPoint((int) pp.x, -(int) pp.y);
        pp = transform.multiplyBy(new Point2D(-0.04, -0.04));
        p.addPoint((int) pp.x, -(int) pp.y);
        pp = transform.multiplyBy(new Point2D(0.12, -0.04));
        p.addPoint((int) pp.x, -(int) pp.y);
        pp = transform.multiplyBy(new Point2D(0.12, 0.0));
        p.addPoint((int) pp.x, -(int) pp.y);
        g.setColor(new Color(100, 100, 100));
        g.fillPolygon(p);
        g.setColor(new Color(0, 0, 0));
        ...
        pp = transform.multiplyBy(new Point2D(-0.05, 0.0));
        p.addPoint((int) pp.x, -(int) pp.y);
        pp = transform.multiplyBy(new Point2D(0.05, 0.0));
        p.addPoint((int) pp.x, -(int) pp.y);
        pp = transform.multiplyBy(new Point2D(0.05, 0.48));
        p.addPoint((int) pp.x, -(int) pp.y);
        pp = transform.multiplyBy(new Point2D(-0.05, 0.48));
        p.addPoint((int) pp.x, -(int) pp.y);
        ...
        if (PushTime > 0) {
            ...
            Point2D p1 = tArrow.multiplyBy(new Point2D(0, 0));
            Point2D p2 = tArrow.multiplyBy(new Point2D(-PushForce * 0.01, 0));
            g.drawLine((int) p1.x, -(int) p1.y, (int) p2.x, -(int) p2.y);

            p2 = tArrow.multiplyBy(new Point2D(-sign(PushForce) * 0.05, 0.05));
            g.drawLine((int) p1.x, -(int) p1.y, (int) p2.x, -(int) p2.y);
    ...
    public void dynBip7Eqns(float[] st) {
        ...
        k[87]     = cos_cm7 * LENCM7;
        k[88]     = -Fext[2] - Fext[0];
        k[89]     = -Fext[3] - Fext[1] - MASS6 * G;
        k[90]     = -Fext[10] - Fext[8];
        k[91]     = -Fext[11] - Fext[9] - MASS3 * G;
        k[92]     = -MASS2 * G;
        k[93]     = -Fext[6] - Fext[4];
        k[94]     = -Fext[7] - Fext[5] - MASS7 * G;
        k[95]     = -Fext[14] - Fext[12];
        k[96]     = -Fext[15] - Fext[13] - MASS5 * G;
        k[97]     = -MASS4 * G;
        k[98]     = -Fext[22] - Fext[20] - Fext[18] - Fext[16];
        k[99]     = -Fext[23] - Fext[21] - Fext[19] - Fext[17] - MASS1 * G;
        k[100]    = MASS7 * G * LENCM7 * cos_cm7 + t[6] - Fext[6] * LENMON4 * sin_m4 + Fext[7] * LENMON4 * cos_m4 - Fext[4] * LENMON3 * sin_m3 + Fext[5] * LENMON3 * cos_m3;
        k[101]    = -MASS7 * LENCM7 * cos_cm7;
        k[102]    = MASS7 * LENCM7 * sin_cm7;
        k[103]    = MASS6 * G * LENCM6 * cos_cm6 + t[5] - Fext[2] * LENMON2 * sin_m2 + Fext[3] * LENMON2 * cos_m2 - Fext[0] * LENMON1 * sin_m1 + Fext[1] * LENMON1 * cos_m1;
        k[104]    = -MASS6 * LENCM6 * cos_cm6;
        k[105]    = MASS6 * LENCM6 * sin_cm6;
        k[106]    = MASS5 * G * LENCM5 * cos_cm5 + t[4] - t[6] - Fext[14] * LENMON8 * sin_m8 + Fext[15] * LENMON8 * cos_m8 - Fext[12] * LENMON7 * sin_m7 + Fext[13] * LENMON7 * cos_m7;
        k[107]    = -LENAT7 * cos_at7;
        k[108]    = LENAT7 * sin_at7;
        k[109]    = -MASS5 * LENCM5 * cos_cm5;
        k[110]    = MASS5 * LENCM5 * sin_cm5;
        k[111]    = MASS4 * G * LENCM4 * cos_cm4 + t[3] - t[4];
        k[112]    = -LENAT5 * cos_at5;
        k[113]    = LENAT5 * sin_at5;
        k[114]    = -MASS4 * LENCM4 * cos_cm4;
        k[115]    = MASS4 * LENCM4 * sin_cm4;
        k[116]    = MASS3 * G * LENCM3 * cos_cm3 + t[2] - t[5] - Fext[10] * LENMON6 * sin_m6 + Fext[11] * LENMON6 * cos_m6 - Fext[8] * LENMON5 * sin_m5 + Fext[9] * LENMON5 * cos_m5;
        k[117]    = -LENAT6 * cos_at6;
        k[118]    = LENAT6 * sin_at6;
        k[119]    = -MASS3 * LENCM3 * cos_cm3;
        k[120]    = MASS3 * LENCM3 * sin_cm3;
        k[121]    = MASS2 * G * LENCM2 * cos_cm2 + t[1] - t[2];
        k[122]    = -LENAT3 * cos_at3;
        k[123]    = LENAT3 * sin_at3;
        k[124]    = -MASS2 * LENCM2 * cos_cm2;
        k[125]    = MASS2 * LENCM2 * sin_cm2;
        k[126]    = MASS1 * G * LENCM1 * cos_cm1 + t[0] - t[1] - t[3] - Fext[22] * LENMON12 * sin_m12 + Fext[23] * LENMON12 * cos_m12 - Fext[20] * LENMON11 * sin_m11 + Fext[21] * LENMON11 * cos_m11 - Fext[18] * LENMON10 * sin_m10 + Fext[19] * LENMON10 * cos_m10 - Fext[16] * LENMON9 * sin_m9 + Fext[17] * LENMON9 * cos_m9;
        ...
        k[144]    = MASS7 * k[139];
        k[145]    = MASS7 * k[140];
        k[146]    = MASS7 * k[141] + k[93];
        k[147]    = k[57] + k[64];
        k[148]    = k[133] + k[65];
        k[149]    = k[134] + k[62];
        k[150]    = MASS5 + MASS7;
        k[151]    = MASS5 * k[63] + k[143];
        k[152]    = MASS5 * k[147] + k[144];
        k[153]    = MASS5 * k[148] + k[145];
        k[154]    = MASS5 * k[149] + k[95] + k[146];
        k[155]    = k[47] + k[52];
        k[156]    = k[46] + k[50];
        k[157]    = MASS4 + k[150];
        k[158]    = MASS4 * k[51] + k[152];
        k[159]    = MASS4 * k[155] + k[153];
        k[160]    = MASS4 * k[156] + k[154];
        k[161]    = k[5] + k[16];
        k[162]    = k[4] + k[14];
        k[163]    = k[15] + k[30];
        k[164]    = k[161] + k[31];
        ...
        k[172]    = MASS6 * k[167];
        k[173]    = MASS6 * k[168];
        k[174]    = MASS6 * k[169] + k[88];
        k[175]    = k[15] + k[22];
        k[176]    = k[161] + k[23];
        k[177]    = k[162] + k[20];
        k[178]    = MASS3 + MASS6;
        k[179]    = MASS3 * k[21] + k[171];
        k[180]    = MASS3 * k[175] + k[172];
        k[181]    = MASS3 * k[176] + k[173];
        k[182]    = MASS3 * k[177] + k[90] + k[174];
        k[183]    = k[5] + k[10];
        k[184]    = k[4] + k[8];
        k[185]    = MASS2 + k[178];
        k[186]    = MASS2 * k[9] + k[180];
        k[187]    = MASS2 * k[183] + k[181];
        k[188]    = MASS2 * k[184] + k[182];
        k[189]    = k[49] + k[61];
        k[190]    = k[48] + k[59];
        k[191]    = k[60] + k[76];
        k[192]    = k[189] + k[77];
        ...
        k[200]    = MASS7 * k[195];
        k[201]    = MASS7 * k[196];
        k[202]    = MASS7 * k[197] + k[94];
        k[203]    = k[60] + k[68];
        k[204]    = k[189] + k[69];
        k[205]    = k[190] + k[66];
        k[206]    = MASS5 + MASS7;
        k[207]    = MASS5 * k[67] + k[199];
        k[208]    = MASS5 * k[203] + k[200];
        k[209]    = MASS5 * k[204] + k[201];
        k[210]    = MASS5 * k[205] + k[96] + k[202];
        k[211]    = k[49] + k[55];
        k[212]    = k[48] + k[53];
        k[213]    = MASS4 + k[206];
        k[214]    = MASS4 * k[54] + k[208];
        k[215]    = MASS4 * k[211] + k[209];
        k[216]    = MASS4 * k[212] + k[97] + k[210];
        k[217]    = k[7] + k[19];
        k[218]    = k[6] + k[17];
        k[219]    = k[18] + k[34];
        k[220]    = k[217] + k[35];
        ...
        k[228]    = MASS6 * k[223];
        k[229]    = MASS6 * k[224];
        k[230]    = MASS6 * k[225] + k[89];
        k[231]    = k[18] + k[26];
        k[232]    = k[217] + k[27];
        k[233]    = k[218] + k[24];
        k[234]    = MASS3 + MASS6;
        k[235]    = MASS3 * k[25] + k[227];
        k[236]    = MASS3 * k[231] + k[228];
        k[237]    = MASS3 * k[232] + k[229];
        k[238]    = MASS3 * k[233] + k[91] + k[230];
        k[239]    = k[7] + k[13];
        k[240]    = k[6] + k[11];
        k[241]    = MASS2 + k[234];
        k[242]    = MASS2 * k[12] + k[236];
        k[243]    = MASS2 * k[239] + k[237];
        k[244]    = MASS2 * k[240] + k[92] + k[238];

        A[0][0] = MASS1 + k[185] + k[157];
        A[0][1] = 0;
        A[0][2] = k[142];
        A[0][3] = k[170];
        A[0][4] = k[151];
        ...
        A[0][6] = k[179];
        A[0][7] = k[186];
        A[0][8] = MASS1 * k[1] + k[187] + k[159];
        A[1][0] = 0;
        A[1][1] = MASS1 + k[241] + k[213];
        A[1][2] = k[198];
        A[1][3] = k[226];
        A[1][4] = k[207];
        A[1][5] = k[214];
        A[1][6] = k[235];
        A[1][7] = k[242];
        A[1][8] = MASS1 * k[3] + k[243] + k[215];
        A[2][0] = k[102];
        A[2][1] = k[101];
        A[2][2] = k[102] * k[79] + k[101] * k[84] - INER7;
        A[2][3] = 0;
        A[2][4] = k[102] * k[138] + k[101] * k[194] - INER7;
        A[2][5] = k[102] * k[139] + k[101] * k[195] - INER7;
        A[2][6] = 0;
        A[2][7] = 0;
        A[2][8] = k[102] * k[140] + k[101] * k[196] - INER7;
        A[3][0] = k[105];
        A[3][1] = k[104];
        A[3][2] = 0;
        A[3][3] = k[105] * k[37] + k[104] * k[42] - INER6;
        A[3][4] = 0;
        A[3][5] = 0;
        A[3][6] = k[105] * k[166] + k[104] * k[222] - INER6;
        A[3][7] = k[105] * k[167] + k[104] * k[223] - INER6;
        A[3][8] = k[105] * k[168] + k[104] * k[224] - INER6;
        A[4][0] = k[110] + k[108] * MASS7;
        A[4][1] = k[109] + k[107] * MASS7;
        A[4][2] = k[108] * k[142] + k[107] * k[198];
        A[4][3] = 0;
        A[4][4] = k[110] * k[63] + k[109] * k[67] + k[108] * k[143] + k[107] * k[199] - INER5;
        A[4][5] = k[110] * k[147] + k[109] * k[203] + k[108] * k[144] + k[107] * k[200] - INER5;
        A[4][6] = 0;
        A[4][7] = 0;
        A[4][8] = k[110] * k[148] + k[109] * k[204] + k[108] * k[145] + k[107] * k[201] - INER5;
        A[5][0] = k[115] + k[113] * k[150];
        A[5][1] = k[114] + k[112] * k[206];
        A[5][2] = k[113] * k[142] + k[112] * k[198];
        A[5][3] = 0;
        A[5][4] = k[113] * k[151] + k[112] * k[207];
        A[5][5] = k[115] * k[51] + k[114] * k[54] + k[113] * k[152] + k[112] * k[208] - INER4;
        A[5][6] = 0;
        A[5][7] = 0;
        A[5][8] = k[115] * k[155] + k[114] * k[211] + k[113] * k[153] + k[112] * k[209] - INER4;
        A[6][0] = k[120] + k[118] * MASS6;
        A[6][1] = k[119] + k[117] * MASS6;
        A[6][2] = 0;
        A[6][3] = k[118] * k[170] + k[117] * k[226];
        A[6][4] = 0;
        A[6][5] = 0;
        A[6][6] = k[120] * k[21] + k[119] * k[25] + k[118] * k[171] + k[117] * k[227] - INER3;
        A[6][7] = k[120] * k[175] + k[119] * k[231] + k[118] * k[172] + k[117] * k[228] - INER3;
        A[6][8] = k[120] * k[176] + k[119] * k[232] + k[118] * k[173] + k[117] * k[229] - INER3;
        A[7][0] = k[125] + k[123] * k[178];
        A[7][1] = k[124] + k[122] * k[234];
        A[7][2] = 0;
        A[7][3] = k[123] * k[170] + k[122] * k[226];
        A[7][4] = 0;
        A[7][5] = 0;
        A[7][6] = k[123] * k[179] + k[122] * k[235];
        A[7][7] = k[125] * k[9] + k[124] * k[12] + k[123] * k[180] + k[122] * k[236] - INER2;
        A[7][8] = k[125] * k[183] + k[124] * k[239] + k[123] * k[181] + k[122] * k[237] - INER2;
        A[8][0] = k[132] + k[130] * k[185] + k[128] * k[157];
        A[8][1] = k[131] + k[129] * k[241] + k[127] * k[213];
        A[8][2] = k[128] * k[142] + k[127] * k[198];
        A[8][3] = k[130] * k[170] + k[129] * k[226];
        A[8][4] = k[128] * k[151] + k[127] * k[207];
        A[8][5] = k[128] * k[158] + k[127] * k[214];
        A[8][6] = k[130] * k[179] + k[129] * k[235];
        A[8][7] = k[130] * k[186] + k[129] * k[242];
        A[8][8] = k[132] * k[1] + k[131] * k[3] + k[130] * k[187] + k[129] * k[243] + k[128] * k[159] + k[127] * k[215] - INER1;
        b[0]    = -MASS1 * k[0] - k[98] - k[188] - k[160];
        b[1]    = -MASS1 * k[2] - k[99] - k[244] - k[216];
        b[2]    = -k[102] * k[141] - k[101] * k[197] - k[100];
        b[3]    = -k[105] * k[169] - k[104] * k[225] - k[103];
        b[4]    = -k[110] * k[149] - k[109] * k[205] - k[108] * k[146] - k[107] * k[202] - k[106];
        b[5]    = -k[115] * k[156] - k[114] * k[212] - k[111] - k[113] * k[154] - k[112] * k[210];
        b[6]    = -k[120] * k[177] - k[119] * k[233] - k[118] * k[174] - k[117] * k[230] - k[116];
        b[7]    = -k[125] * k[184] - k[124] * k[240] - k[121] - k[123] * k[182] - k[122] * k[238];
        b[8]    = -k[132] * k[0] - k[131] * k[2] - k[130] * k[188] - k[129] * k[244] - k[128] * k[160] - k[127] * k[216] - k[126];
    ...
    public void monBip7(float[] state, float[] mState) {
        ...
        sin_m12    = (float) Math.sin(m_angle12);
        cos_m12    = (float) Math.cos(m_angle12);
        mState[44] = LENMON12 * cos_m12 + xo1;
        mState[46] = LENMON12 * sin_m12 + yo1;
        mState[45] = -LENMON12 * sin_m12 * dth1 + vxo1;
        mState[47] = LENMON12 * cos_m12 * dth1 + vyo1;
        m_angle11  = A_M11 + th1;
        sin_m11    = (float) Math.sin(m_angle11);
        cos_m11    = (float) Math.cos(m_angle11);
        mState[40] = LENMON11 * cos_m11 + xo1;
        mState[42] = LENMON11 * sin_m11 + yo1;
        mState[41] = -LENMON11 * sin_m11 * dth1 + vxo1;
        mState[43] = LENMON11 * cos_m11 * dth1 + vyo1;
        m_angle10  = A_M10 + th1;
        sin_m10    = (float) Math.sin(m_angle10);
        cos_m10    = (float) Math.cos(m_angle10);
        mState[36] = LENMON10 * cos_m10 + xo1;
        mState[38] = LENMON10 * sin_m10 + yo1;
        mState[37] = -LENMON10 * sin_m10 * dth1 + vxo1;
        mState[39] = LENMON10 * cos_m10 * dth1 + vyo1;
        m_angle9   = A_M9 + th1;
        sin_m9     = (float) Math.sin(m_angle9);
        cos_m9     = (float) Math.cos(m_angle9);
        mState[32] = LENMON9 * cos_m9 + xo1;
        mState[34] = LENMON9 * sin_m9 + yo1;
        mState[33] = -LENMON9 * sin_m9 * dth1 + vxo1;
        mState[35] = LENMON9 * cos_m9 * dth1 + vyo1;
        th2        = th1 + state[6];
        dth2       = dth1 + state[7];
        at_angle2  = A_AT2 + th1;
        sin_at2    = (float) Math.sin(at_angle2);
        cos_at2    = (float) Math.cos(at_angle2);
        xo2        = LENAT2 * cos_at2 + xo1;
        yo2        = LENAT2 * sin_at2 + yo1;
        vxo2       = -LENAT2 * sin_at2 * dth1 + vxo1;
        vyo2       = LENAT2 * cos_at2 * dth1 + vyo1;
        th3        = th2 + state[8];
        dth3       = dth2 + state[9];
        at_angle3  = A_AT3 + th2;
        sin_at3    = (float) Math.sin(at_angle3);
        cos_at3    = (float) Math.cos(at_angle3);
        xo3        = LENAT3 * cos_at3 + xo2;
        yo3        = LENAT3 * sin_at3 + yo2;
        vxo3       = -LENAT3 * sin_at3 * dth2 + vxo2;
        vyo3       = LENAT3 * cos_at3 * dth2 + vyo2;
        m_angle6   = A_M6 + th3;
        sin_m6     = (float) Math.sin(m_angle6);
        cos_m6     = (float) Math.cos(m_angle6);
        mState[20] = LENMON6 * cos_m6 + xo3;
        mState[22] = LENMON6 * sin_m6 + yo3;
        mState[21] = -LENMON6 * sin_m6 * dth3 + vxo3;
        mState[23] = LENMON6 * cos_m6 * dth3 + vyo3;
        m_angle5   = A_M5 + th3;
        sin_m5     = (float) Math.sin(m_angle5);
        cos_m5     = (float) Math.cos(m_angle5);
        mState[16] = LENMON5 * cos_m5 + xo3;
        mState[18] = LENMON5 * sin_m5 + yo3;
        mState[17] = -LENMON5 * sin_m5 * dth3 + vxo3;
        mState[19] = LENMON5 * cos_m5 * dth3 + vyo3;
        th6        = th3 + state[14];
        dth6       = dth3 + state[15];
        at_angle6  = A_AT6 + th3;
        sin_at6    = (float) Math.sin(at_angle6);
        cos_at6    = (float) Math.cos(at_angle6);
        xo6        = LENAT6 * cos_at6 + xo3;
        yo6        = LENAT6 * sin_at6 + yo3;
        vxo6       = -LENAT6 * sin_at6 * dth3 + vxo3;
        vyo6       = LENAT6 * cos_at6 * dth3 + vyo3;
        m_angle2   = A_M2 + th6;
        sin_m2     = (float) Math.sin(m_angle2);
        cos_m2     = (float) Math.cos(m_angle2);
        mState[4]  = LENMON2 * cos_m2 + xo6;
        mState[6]  = LENMON2 * sin_m2 + yo6;
        mState[5]  = -LENMON2 * sin_m2 * dth6 + vxo6;
        mState[7]  = LENMON2 * cos_m2 * dth6 + vyo6;
        m_angle1   = A_M1 + th6;
        sin_m1     = (float) Math.sin(m_angle1);
        cos_m1     = (float) Math.cos(m_angle1);
        mState[0]  = LENMON1 * cos_m1 + xo6;
        mState[2]  = LENMON1 * sin_m1 + yo6;
        mState[1]  = -LENMON1 * sin_m1 * dth6 + vxo6;
        mState[3]  = LENMON1 * cos_m1 * dth6 + vyo6;
        th4        = th1 + state[10];
        dth4       = dth1 + state[11];
        at_angle4  = A_AT4 + th1;
        sin_at4    = (float) Math.sin(at_angle4);
        cos_at4    = (float) Math.cos(at_angle4);
        xo4        = LENAT4 * cos_at4 + xo1;
        yo4        = LENAT4 * sin_at4 + yo1;
        vxo4       = -LENAT4 * sin_at4 * dth1 + vxo1;
        vyo4       = LENAT4 * cos_at4 * dth1 + vyo1;
        th5        = th4 + state[12];
        dth5       = dth4 + state[13];
        at_angle5  = A_AT5 + th4;
        sin_at5    = (float) Math.sin(at_angle5);
        cos_at5    = (float) Math.cos(at_angle5);
        xo5        = LENAT5 * cos_at5 + xo4;
        yo5        = LENAT5 * sin_at5 + yo4;
        vxo5       = -LENAT5 * sin_at5 * dth4 + vxo4;
        vyo5       = LENAT5 * cos_at5 * dth4 + vyo4;
        m_angle8   = A_M8 + th5;
        sin_m8     = (float) Math.sin(m_angle8);
        cos_m8     = (float) Math.cos(m_angle8);
        mState[28] = LENMON8 * cos_m8 + xo5;
        mState[30] = LENMON8 * sin_m8 + yo5;
        mState[29] = -LENMON8 * sin_m8 * dth5 + vxo5;
        mState[31] = LENMON8 * cos_m8 * dth5 + vyo5;
        m_angle7   = A_M7 + th5;
        sin_m7     = (float) Math.sin(m_angle7);
        cos_m7     = (float) Math.cos(m_angle7);
        mState[24] = LENMON7 * cos_m7 + xo5;
        mState[26] = LENMON7 * sin_m7 + yo5;
        mState[25] = -LENMON7 * sin_m7 * dth5 + vxo5;
        mState[27] = LENMON7 * cos_m7 * dth5 + vyo5;
        th7        = th5 + state[16];
        dth7       = dth5 + state[17];
        at_angle7  = A_AT7 + th5;
        sin_at7    = (float) Math.sin(at_angle7);
        cos_at7    = (float) Math.cos(at_angle7);
        xo7        = LENAT7 * cos_at7 + xo5;
        yo7        = LENAT7 * sin_at7 + yo5;
        vxo7       = -LENAT7 * sin_at7 * dth5 + vxo5;
        vyo7       = LENAT7 * cos_at7 * dth5 + vyo5;
        m_angle4   = A_M4 + th7;
        sin_m4     = (float) Math.sin(m_angle4);
        cos_m4     = (float) Math.cos(m_angle4);
        mState[12] = LENMON4 * cos_m4 + xo7;
        mState[14] = LENMON4 * sin_m4 + yo7;
        mState[13] = -LENMON4 * sin_m4 * dth7 + vxo7;
        mState[15] = LENMON4 * cos_m4 * dth7 + vyo7;
        m_angle3   = A_M3 + th7;
        sin_m3     = (float) Math.sin(m_angle3);
        cos_m3     = (float) Math.cos(m_angle3);
        mState[8]  = LENMON3 * cos_m3 + xo7;
        mState[10] = LENMON3 * sin_m3 + yo7;
        mState[9]  = -LENMON3 * sin_m3 * dth7 + vxo7;
        mState[11] = LENMON3 * cos_m3 * dth7 + vyo7;
    }
